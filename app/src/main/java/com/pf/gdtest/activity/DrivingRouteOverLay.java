...
public class DrivingRouteOverLay extends RouteOverlay {
    ...
    public void zoomToSpan() {
        if (startPoint != null) {
            ...
            try {
                List<LatLng> latLngList = mPolylineOptions.getPoints();
                if (latLngList != null && latLngList.size() > 0) {
                    // 设置屏幕中心点和缩放
                    mAMap.moveCamera(CameraUpdateFactory.newLatLngZoom(MapUtil.getCenterLatlng(latLngList),
                            MapUtil.getScaleLevel(latLngList)));
                } else {
    ...
    private void initPolylineOptions() {
        ...
        mPolylineOptions.color(getDriveColor());
    ...
    private void colorWayUpdate(List<TMC> tmcSection) {
        ...
        addPolyLine(new PolylineOptions().add(startPoint,
                MapUtil.convertToLatLng(tmcSection.get(0).getPolyline().get(0)))
                .setDottedLine(true));
        String status = "";
        for (int i = 0; i < tmcSection.size(); i++) {
            ...
            if (i == tmcSection.size() - 1 && mPolylineOptionscolor != null) {
                ...
                addPolyLine(new PolylineOptions().add(
                        MapUtil.convertToLatLng(mployline.get(mployline.size() - 1)), endPoint)
                        .setDottedLine(true));
            }
        }
    }
